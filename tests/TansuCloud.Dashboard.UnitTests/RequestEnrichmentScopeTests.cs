...
{
	[Fact(DisplayName = "Dashboard scope contains correlation/tenant/route/trace/span and echoes header")]
	public async Task Scope_And_Header_Echo_Work()
	{
		...
		var builder = new WebHostBuilder()
			...
			.Configure(app =>
			{
				...
				{
					endpoints.MapGet("/__scope-probe", async context =>
					{
	...
	// Reuse the simple scope-capturing provider
	private sealed class ScopeCapturingLoggerProvider : ILoggerProvider
	{
		...
		public List<(string Category, LogLevel Level, EventId Id, string Message, object? Scope)> Events { get; } = new();
		public ILogger CreateLogger(string categoryName) => _loggers.GetOrAdd(categoryName, c => new ScopeLogger(c, this));
		public void Dispose() { }
		...
		private sealed class ScopeLogger : ILogger
		{
			...
			public ScopeLogger(string category, ScopeCapturingLoggerProvider owner) { _category = category; _owner = owner; }
			...
			public void Log<TState>(LogLevel logLevel, EventId eventId, TState state, Exception? exception, Func<TState, Exception?, string> formatter)
			{
				...
				_owner.Events.Add((_category, logLevel, eventId, formatter(state, exception), current));
			}
